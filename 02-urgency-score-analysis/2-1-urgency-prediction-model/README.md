...
| 라벨 | **v5** (`master_merged_v5.json`) |
| 학습 데이터 | measurable, test 6,056행 |
| 예측 방식 | 클래스 확률의 **기댓값 반올림** (validation에서 확정) |
| 파일 | `models_v5/urgency_model.joblib` |

### 성능 (measurable hold-out 6,056행, 최종 1회 평가)

| 지표 | 상수(2점) | **v5 모델** |
|---|---|---|
| **MAE** (주 지표) | 0.4775 | **0.3109** (−34.9%) |
| **QWK** (주 지표) | 0.0000 | **0.6120** |
| 인접 오차 내 정확도 (±1점) | 97.81% | **98.98%** |
| Macro F1 | — | 0.5501 |
| Accuracy | — | 0.7003 |

> ⚠️ **v4 열(0.3131 / 0.6098)과 이 표를 "좋아졌다"로 읽지 말 것.** v5는 v4와
> 라벨이 **38행(0.09%)** 다를 뿐이고, 이 차이는 학습 변동 수준이다. 조건을
> 고정해 비교 가능한 전이 실험에서는 오히려 **세 지표 모두 미세하게 내려갔다**
> — 전이 QWK 0.1656 → 0.1627, 상수 대비 QWK 0.1689 → 0.1662, 상수 대비 MAE
> −29.6% → −31.1%. v5가 고친 것은 성능이 아니라 라벨의 정확성이다.

라벨이 1~5 순서형이므로 QWK와 MAE를 주 지표로 삼았다. "1점을 5점으로" 틀린 것과
"3점을 4점으로" 틀린 것은 같은 오류가 아니다. **예측의 99.0%가 실제 값의 ±1점
안에 들어온다.**

> ⚠️ **아래 문서에 나오는 v1/v2/v3/v4 수치와 이 표를 직접 비교하지 말 것.**
...
>
> 이전 버전 모델은 비교용으로 보존돼 있다 — `models_v4/`(v4), `models_v3/`(v3),
> `models_transfer/`(v2).
...
- 학습에 없던 채용 사이트 — cross-source 전이가 실패한다 (v5에서도 상수보다 MAE가 나쁘다, 아래 참조)

> **먼저 규칙을 고려할 것.** 메타데이터가 있는 공고라면
> [`../urgency_rule.py`](../urgency_rule.py)(v5)를 그대로 돌리는 편이 낫다.
...
> v2 시절 인터페이스**다. v5 점수를 원하면 아래 `UrgencyModel`을 쓸 것.

```python
import sys; sys.path.insert(0, '..')
from urgency_model import UrgencyModel

p = UrgencyModel()                      # models_v5/ 우선, 없으면 v4 -> v3
...
라벨 `urgency_score`는 규칙(당시 `rescore_urgency.py` = v2, 현재 `../urgency_rule.py` = v5)이
...
> 현재 정본은 `models_v5/`이며, 그 수치는 이 문서 맨 위 "최종 모델 (v5)" 절에 있다.
...
**v5에서는 둘 다 조금 내려갔다**(MAE 개선 −31.1%, QWK 0.1662). 라벨이 38행
(0.09%)만 바뀐 라운드라 학습 변동 수준이지만, 올랐다고 적을 수 없으므로
내려간 그대로 적는다. [수정 5]가 고친 것은 성능이 아니라 라벨의 정확성이다.
...
`models_<라운드>_struct/`에 저장한다(`--v5`면 `models_v5_struct/`). 정본
**`models_v5/`**는 통제가 걸린 모델로 유지된다 — 위 수치는 v3 라운드에서 잰 것이다.
...
- **`채용 시 마감`이 두 신호에 중복 계상된다.** rolling(+10)과 조기마감(+12)에
  ...
  **재학습도 끝났다**(`models_v4/`, 690.6초). 아래 성능 수치는 v3 라벨 기준의 기록이다.
- **상시채용 판정이 본문 전체를 봤다.** v4까지 이 파일에서 유일하게 앵커 없이
  `RX_ALWAYS_OPEN`을 본문에 돌리던 자리다.
  → **v5에서 고쳤다**(`is_always_open()`으로 교체). 다만 `win is None` 가드가
  이미 오탐 대부분을 막고 있어서 실제 라벨 변경은 **38행(0.09%)** 뿐이다.
  정본은 `models_v5/`(827.8초)이고 성능은 맨 위 "최종 모델 (v5)" 절에 있다.
...
**모델**
- `models_v5/` — **현재 최종 모델. 앱이 로드하는 정본.** v5 라벨, measurable 학습, 기댓값 예측
- `models_v4/` — v4 최종 모델 (비교 기준으로 보존)
...
```bash
# v5 (현재)
python ../urgency_rule.py --write       # 라벨 재생성 (약 2분) -> data/master_merged_v5.json
                                        #   HF에도 있으므로 생략 가능(그러면 fetch가 받는다)
python ../test_urgency_rule.py          # 규칙 회귀 테스트 23종
python calibrate_fallback.py --grid     # 폴백 재보정 근거 (약 1분)
python train_urgency.py --v5            # 학습 + v4 대비 전이 비교 (약 14분) -> models_v5/
python compare_labels.py --v5           # 상수 대비 재측정 (약 1분)
# 라벨을 바꿨으면 ../2-2-urgency-app/build_reference_stats.py --write 도 함께 돌릴 것

# v4 · v3 (보존용 재현)
python train_urgency.py --v4            # -> models_v4/
python compare_labels.py --v4
python train_urgency.py                 # 학습 + v2 대비 전이 비교 -> models_v3/
python compare_labels.py                # 상수 대비 재측정

# 구조화 피처 실험 (배포용 아님, models_v3_struct/ 로 나간다. --v5 면 models_v5_struct/)
